...
const PredictiveMaintenance: React.FC<ActionDetectorProps> = ({ thresholds, deviceId, astarteClient }) => {
    ...
    useEffect(() => {
        ...
        const dataHandler = (data: any) => {
            ...
            if (data.event == "new_event" && data.payload.event.type == "incoming_data") {
                let event = data.payload.event
                console.log(data.payload)
                //console.log(`new value -> ${event.path}:\n\t${event.value}`)
                let newValue = {
                    value: event.value,
                ...
                switch (event.path) {
                    case "/flow":
                        ...
                        break;
                    case "/velocity":
                        setAirVelocity(newValue);
                        break;
                    case "/pollution":
                        setAirPollution(newValue);
                        break;
        ...
        astarteClient.apiHandler.connectSocket({
            device: deviceId, interfaceName: "ai.clea.examples.AirData",
            interfaceMajor: 0, onInComingData: dataHandler
        })
        ...
        return () => {
            ...
            astarteClient.apiHandler.disconnectSocket()
        };
        ...
        try {
            const [latestAirFlowValues, latestAirPollutionValues, latestAirVelocityValues] = await Promise.all([
                astarteClient.getAirFlowValues({
                    deviceId,
                    limit: 1,
                    since: moment().subtract(10, 'seconds').toDate()
                }),
    ...
    const getLatestFluxEventsHistoryValues = async ({ numEvents, maxEventsToShow = 50 }: { numEvents: number; maxEventsToShow: number }) => {
        const latestFluxEventsHistoryValues = await astarteClient.getFluxEventsHistoryValues({
            deviceId, endpoint: "event",
            limit: numEvents,
