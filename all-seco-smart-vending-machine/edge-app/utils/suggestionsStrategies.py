    ...
    def suggest_promo(self, session, available_promos:dict) -> typing.Optional[str]:
        def promo_filter(p, s):
            ...
            try :
                ...
                self.__logger.error(f"Cannot filter promo: {e}")
                return False
        ...
        def adv_filter(a, s):
            ...
            try :
                ...
                if a["currentStatus"].lower()!="active" or (not self.__check_schedule_time(a["startScheduleTime"], a["endScheduleTime"])) or \
                    ...
                    return False
                # Checking age and emotion
                if a["targetedAdv"]==True:
                    if (not self.__check_inference_results(session, a["minAge"], a["maxAge"], a["emotions"])):
                        self.__logger.debug("Requirements not satisfied!")
                        return False
            except Exception as e:
                self.__logger.error(f"Cannot filter advertisement: {e}")
                return False
            return True
