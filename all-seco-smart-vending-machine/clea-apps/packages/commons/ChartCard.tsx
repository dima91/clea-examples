...
export type GetDataCallback     = (astarte:AstarteInterface, since:moment.Moment, to:moment.Moment) => any;
export type DataFilterCallback  = (items:any) => any;
...
export const ChartCard : React.FC<ChartCardProps> = ({astarte, chart_name, data_retriever_cb, data_filter_cb}:ChartCardProps) => {
    ...
    const resize_chart  = () => {
        if (chart_container_reference.current) {
            ...
            let newWidth    = domRect.width*WIDTH_SCALE_FACTOR
            let newHeight   = domRect.height*HEIGHT_SCALE_FACTOR
    ...
    <Card className="m-1 shadow">
        <Card.Body>
            ...
            <ButtonToolbar className="d-flex justify-content-end m-2">
                ...
                {/*DATE PICKER*/}
                <div>
                    <style>{Commons.get_datepicker_style()}</style> {/*FIXME*/}
                    <DatePicker
                        selectsRange={true}
            ...
            <div className="d-flex justify-content-end pe-3 fs-6 m-3">
                {range_to_string()}
            </div>
            
            
            <Container fluid ref={chart_container_reference}>
                <Col sm={12} md={12}>
