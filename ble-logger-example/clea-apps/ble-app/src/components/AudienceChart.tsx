...
const AudienceChart = ({ data: { presence, trend }, xRange }: Props) => {
  const intl = useIntl();
    ...
    return [
      {
        name: intl.formatMessage({
          id: "audience-chart-presence",
          defaultMessage: "Presence",
        }),
        data: presence.map((p) => [new Date(p.timestamp), p.value]),
        type: "column",
      },
      {
        name: intl.formatMessage({
          id: "audience-chart-trend",
          defaultMessage: "Trend",
        }),
        data: trend.map((t) => [new Date(t.timestamp), t.value]),
      },
    ];
  }, [presence, trend]);
